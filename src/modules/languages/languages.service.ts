import { HttpException } from "@core/exceptions";
import ILanguage from "./languages.interface";
import LanguageSchema from "./languages.model"
import LanguageRequestDto from "./dtos/language.request.dto";
import { IPagination } from "@core/interfaces";

const modelName = "Language";

class LanguageService {
    public modelSchema = LanguageSchema;

    public async create(model: LanguageRequestDto): Promise<ILanguage> {
        const newItem = new LanguageSchema({
            code: model.code,
            isDefault: model.isDefault,
            name: model.name,
        });
        const item = await newItem.save();
        return item;
    }

    public async update(id: string, model: LanguageRequestDto): Promise<ILanguage>{
        const updateId = await this.modelSchema.findByIdAndUpdate(
            id,
            {
                ...model
            },
            {
                new: true
            }
        ).exec();
        if(!updateId) throw new HttpException(400, `${modelName} is not found`);

        return updateId;
    }

    public async getAll(): Promise<ILanguage[]>{
        const items = await this.modelSchema.find().sort({updatedDate:-1}).exec();
        return items;
    }

    public async getById(id: string): Promise<ILanguage>{
        const item = await this.modelSchema.findById(id).exec();
        if(!item) throw new HttpException(404, `${modelName} is not found`);
        return item;
    }

    public async getAllPaging(keyword: string, page: number): Promise<IPagination<ILanguage>>{
        const pageSize: number = Number(process.env.PAGE_SIZE || 10);

        let query = {};
        if(keyword){
            query = {
                $or: [{text: keyword}]
            };
        }

        const items = await this.modelSchema.find(query)
            .skip((page - 1) * pageSize)
            .limit(pageSize)
            .exec();

        const rowCount = await this.modelSchema.find(query).countDocuments().exec();

        return {
            total: rowCount,
            page,
            pageSize,
            items
        } as IPagination<ILanguage>;
    }

    public async remove(id: string): Promise<ILanguage>{
        const item = await this.modelSchema.findById(id).exec();
        if(!item) throw new HttpException(400, `${modelName} is not found`);
        await item.deleteOne();
        return item;
    }
}

export default LanguageService;